...
global worldName := "PagCube" ; the world name

global countAttempts := "yes" ; yes or no, counts attempts

global runningMode := "SSG" ; SSG, FSG or RSG
TabAndWait() {
...
TypeSeed() {
	...
	if (runningMode = "SSG") {
		SendInput, %seed%
		Finish()
	} else if (runningMode = "RSG") {
		Finish()
	} else if (runningMode = "FSG") {
		Send ^v
		Finish()
	} else {
	
		MsgBox, Your running mode is invalid. Set it to either SSG, RSG, or FSG at the top of this page.
